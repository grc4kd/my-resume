...
function setupAppCheck() {
  if (!environment.useAppCheck && environment.production) {
    console.warn(`Firebase App "${app.name}": App Check has been disabled. Please turn it back on.`);
  }
  ...
  if (environment.useAppCheck) {
    ...
    if (environment.useCIAppCheckDebugToken) {
      ...
      if (Object.keys(globalThis).includes("APP_CHECK_DEBUG_TOKEN_FROM_CI")) {
        const appCheckDebugTokenFromCI = Object.values(globalThis).find(gb => gb === "APP_CHECK_DEBUG_TOKEN_FROM_CI");
        ...
        if (typeof appCheckDebugTokenFromCI === 'string' &&
          ...
          console.info('The App Check debug token was loaded from the CI environment.');
        }
        else {
          console.error('The debug token could not be loaded from the CI environment.');
        }
      }
    }
